...
\begin{frame}
	...
	\begin{itemize}[<+->]
		...
		\item Conclude that $ C $ is unsatisfiable since every model of $ \varphi $ cannot be a model of $ \{\lambda_k \mid k \in \N\} $ and vice versa.
		\item Show that, on the other hand, for every finite $ M \subseteq C $, there is a model with property $ P $.
		\item Conclude that, by compactness, $ C $ is satisfiable, which is a contradiction to the unsatisfiablity of $ C $.
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{proof}[\proofname\ (continued)]
		\begin{itemize}[<+->]
			...
			\] Since $ M $ is finite, we can find a $ k \in \N $ such that $ M $ doesn't contain any $ \lambda_i $ for $ i \ge k $. Any structure with at least $ k $ elements thus satisfies $ M $.
...
\begin{frame}
	...
	\begin{theorem}[Ramsey Theorem]
		Every infinite graph has either an infinite clique or and infinite independent set.
	\end{theorem}

	\pause

	Assuming this theorem holds, we can use compactness to show the following finite version of Ramsey's theorem:
...
\begin{frame}
	\frametitle{Finitary Ramsey Theorem}
	\begin{proofs}
		Suppose, for a sake of contradiction, that $ \exists k \in \Nsimpl \quad \forall n \in \Nsimpl $ there exists a graph with $ n $ vertices that has no $ k $-clique and no $ k $-independent set. We describe this using an infinite set of formulas $ \Sigma $: \begin{align*}
			\alpha_k &:= \underbrace{\forall v_1, \dots, v_k \Bigg(\bigwedge_{\substack{i,j = 1 \\ i \neq j}}^k v_i \neq v_j\Bigg) \rightarrow \Bigg(\bigvee_{\substack{i,j = 1 \\ i \neq j}}^k \neg E(v_i, v_j)\Bigg) \wedge \Bigg(\bigvee_{\substack{i,j = 1 \\ i \neq j}}^k E(v_i, v_j)\Bigg)}_{\text{There is no clique or independent-set of size } k} \\
...
\begin{frame}
	...
	\begin{proof}[\proofname\ (continued)]
		\begin{itemize}[<+->]
			...
			\item It follows that there exists an infinite graph with no infinite clique and no infinite independent set. This contradicts Ramsey's Theorem.
		\end{itemize}
	\end{proof}
\end{frame}
