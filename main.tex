...
\begin{frame}
	...
	\begin{itemize}[<+->]
		...
		\item \textit{Construct} an infinite set of formulas (e.g., $ \{\lambda_k \mid k \in \Nsimpl\} $) that describes the negation of $ P $.
		\item \textit{Consider} the set $ C := \{\varphi\} \cup \{\lambda_k \mid k \in \Nsimpl\} $.
		\item \textit{Argue} that $ C $ is unsatisfiable since every model of $ \varphi $ cannot be a model of $ \{\lambda_k \mid k \in \Nsimpl\} $ and vice versa.
