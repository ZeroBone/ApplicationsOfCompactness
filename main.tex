...
\begin{frame}
	...
	\begin{itemize}[<+->]
		\item Assume, for a sake of contradiction that there is some formula $ \varphi $ expressing $ P $.
		\item Construct an infinite set of formulas $ \{\lambda_k \mid k \in \N\} $ that ``approximates'' precisely the negation of $ P $.
...
\begin{frame}
	...
	\begin{proofs}[\proofname\ attempt (incorrect)]
		\begin{itemize}
			\item Suppose we have such a formula $ \varphi $. We ``approximate existence of cycles'' by considering $ \{\lambda_k \mid k \in \N\} $ where \[\lambda_k := \underbrace{\exists x_1, \dots, x_k : E(x_1, x_2) \wedge \dots \wedge E(x_{k-1}, x_k) \wedge E(x_k, x_1)}_{\text{There is a cycle of length } k}\]
			\item \[
...
\begin{frame}
	...
	\begin{proofs}[\proofname\ attempt (incorrect) - continued]
		\begin{itemize}
			\item Consider some finite subset \[
			M \subseteq C = \underbrace{\{\varphi\}}_{\text{Graph acyclic}} \cup \quad \underbrace{\{\lambda_k \mid k \in \Npos\}}_{\text{There is a cycle}}
			\]
...
\begin{frame}
	...
	\begin{theorem}[Equivalent version of previous Theorem]
...
\begin{frame}
	\begin{proof}
		\begin{itemize}
			\item Suppose we have such a formula $ \varphi $. We ``approximate acyclicity'' by considering $ \{\lambda_k \mid k \in \N\} $ where \[\lambda_k := \underbrace{\neg \exists x_1, \dots, x_k : E(x_1, x_2) \wedge \dots \wedge E(x_{k-1}, x_k) \wedge E(x_k, x_1)}_{\text{There is no cycle of length } k}\]\[
			C := \underbrace{\underbrace{\{\varphi\}}_{\exists\text{ cycle}} \cup \quad \underbrace{\{\lambda_k \mid k \in \Npos\}}_{\text{Graph is acyclic}}}_{\Rightarrow \text{ unsatisfiable}}
			\]
			\item Consider an arbitrary but finite $ M \subseteq C $. Since $ M $ is finite, we can find a $ k \in \N $ such that $ M $ does not contain any $ \lambda_i $ for $ i \ge k $. We can easily construct a graph with a cycle of length $ k $ and without shorter cycles, meaning that $ M $ is satisfiable.
